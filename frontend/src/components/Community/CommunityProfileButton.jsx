        ...
        <div className={`${className}`}>
            {joined ?
                <>
                    ...
                    <button onClick={onPost} className={`flex p-2 rounded-full ${buttonState ? 'bg-primary-900' : 'bg-gray-500 pointer-events-none'} text-white mr-2 items-center justify-center h-8 w-8 md:h-9 md:w-9 lg:h-10 lg:w-10`} disabled={buttonState == true}><IoNotificationsOffOutline size={'24'} className='text-sm md:text-base lg:text-lg'/></button>
                ...
                :
                <button onClick={onJoin} className={`block p-2 min-w-24 rounded-full ${buttonState ? 'bg-primary-900' : 'bg-gray-500 pointer-events-none'} text-white h-8`} disabled={!buttonState}>Join</button>
            }
        </div>
    )
}
