...
const EditCommunity = () => {
    ...
    const handleCreate = async () => {
        try {
            let res = (await axiosInstance.put(`/communities/${community_id}`, {
                name: title,
    ...
    const getCommunity = async () => {
        try {
            const post = (await axiosInstance.get(`/communities/${community_id}`)).data.data
        ...
        <div className="border p-4 bg-white shadow-md flex flex-col">
            ...
            <div className="relative flex flex-row -mb-12 md:-mb-18 lg:-mb-24 rounded-full">
                <div className="z-20 -translate-y-12 h-24 w-24 p-1 md:-translate-y-18 md:h-36 md:w-36 md:p-2 lg:-translate-y-24 lg:h-48 lg:w-48 lg:p-2 aspect-square object-cover rounded-full bg-white">
                    <img className='rounded-full w-full h-full object-cover' src={`${import.meta.env.VITE_CDN}/uploads${profile ? `/tmp/${profile}` : oldProfile ? `/community/${oldProfile}` : '/community/default_profile.png'}`} alt="" />
